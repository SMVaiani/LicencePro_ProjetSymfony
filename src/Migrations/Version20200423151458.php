 ...
 */
final class Version20200423151458 extends AbstractMigration
{
    ...
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('DROP INDEX IDX_26A98456900D23F1');
        $this->addSql('CREATE TEMPORARY TABLE __temp__achat AS SELECT id, utilisateur_id, packjetons_id, date_achat FROM achat');
        $this->addSql('DROP TABLE achat');
        $this->addSql('CREATE TABLE achat (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, utilisateur_id INTEGER NOT NULL, packjetons_id INTEGER NOT NULL, date_achat DATE NOT NULL, CONSTRAINT FK_26A98456FB88E14F FOREIGN KEY (utilisateur_id) REFERENCES utilisateur (id) NOT DEFERRABLE INITIALLY IMMEDIATE, CONSTRAINT FK_26A98456900D23F1 FOREIGN KEY (packjetons_id) REFERENCES pack_jetons (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO achat (id, utilisateur_id, packjetons_id, date_achat) SELECT id, utilisateur_id, packjetons_id, date_achat FROM __temp__achat');
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__historique_encheres AS SELECT id, utilisateur_id, enchere_id, prix, date_enchere FROM historique_encheres');
        $this->addSql('DROP TABLE historique_encheres');
        $this->addSql('CREATE TABLE historique_encheres (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, utilisateur_id INTEGER NOT NULL, enchere_id INTEGER NOT NULL, prix INTEGER NOT NULL, date_enchere DATETIME NOT NULL, CONSTRAINT FK_4B517BEFFB88E14F FOREIGN KEY (utilisateur_id) REFERENCES utilisateur (id) NOT DEFERRABLE INITIALLY IMMEDIATE, CONSTRAINT FK_4B517BEFE80B6EFB FOREIGN KEY (enchere_id) REFERENCES enchere (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO historique_encheres (id, utilisateur_id, enchere_id, prix, date_enchere) SELECT id, utilisateur_id, enchere_id, prix, date_enchere FROM __temp__historique_encheres');
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__utilisateur AS SELECT id, role_id, email, roles, password, pseudo FROM utilisateur');
        ...
        , password VARCHAR(255) NOT NULL COLLATE BINARY, pseudo VARCHAR(20) NOT NULL COLLATE BINARY, CONSTRAINT FK_1D1C63B3D60322AC FOREIGN KEY (role_id) REFERENCES role (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO utilisateur (id, role_id, email, roles, password, pseudo) SELECT id, role_id, email, roles, password, pseudo FROM __temp__utilisateur');
